...
const coroutines = {
    ...
    filterOutSensitiveInfo: BluebirdPromise.coroutine(function * (data, canAccessSensitiveData) {
        ...
        let arrData = []; let idDataType; let typeIds; let forbiddenField; let forbiddenFields = [];
    ...
    filterListByPrivileges: BluebirdPromise.coroutine(function * (data, dataTypesId, privileges, canAccessSensitiveData) {
        ...
        // or exists at least a VIEW_OVERVIEW privilege level filter metadata
            ...
            let index = 0; let arrDtPrivId = arrPrivileges.map(el => el.dataType);
    ...
    preprocessQueryParams: BluebirdPromise.coroutine(function * (queryArgs, idGroups, idDataType) {
        let dataTypes = [];
        ...
        return { queryObj: queryObj, multiProject: queryArgs.multiProject, leafSearch: queryArgs.leafSearch, dataTypes: dataTypes, dataTypePrivileges: dataTypePrivileges, forbiddenFields: forbiddenFields, forbiddenMetadata: forbiddenMetadata };
    }),
...
let DataService = BluebirdPromise.promisifyAll({
    ...
    buildMetadataFieldValidationSchema: function (metadataField) {
        let fieldValidatorSchema = Joi.object(); let value; let group;
    ...
    */
    executeAdvancedQuery: function (processedArgs, operator, next) {
        ...
        const dataTypePrivileges = processedArgs.dataTypePrivileges;
        const queryObj = processedArgs.queryObj;
        const forbiddenFields = processedArgs.forbiddenFields;
        const forbiddenMetadata = processedArgs.forbiddenMetadata;
        const multiProject = processedArgs.multiProject;
        const leafSearch = processedArgs.leafSearch;
