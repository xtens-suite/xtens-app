       ...
       */
        ...
        displayDaemonsTable: function() {
            ...
            this.tableOpts = {
                data:           this.daemons,
                columns:        [
                  {title: i18n("status"), data: "status"},
                  {title: i18n("file-name"), data: "source"},
                  {title: i18n("progress"), data: "info.percentage", render: function (data, type, row) {
                      return data + "%";
                  }},
                  {title: i18n("imported-rows"), data: "info.processedRows"},
                  {title: i18n("discarded-rows"), data: "info.notProcessedRows", render: function (data, type, row) {
                      return data.length;
                  }},
                  {title: i18n("total-rows"), data: "info.totalRows"},
                  {title: i18n("error"), data: "info.error"},
                  {title: i18n("date"), data: "createdAt", render:function (data, type, row) {
                      return moment(data).format("YYYY-MM-DD HH:mm:ss");
                  } },
                  {title:"process-id", data: "pid", visible: false}
                ],
            ...
            if (notRows.length > 0) {
                ...
                $('td', row).eq(4).on('click', function (e) {
                    ...
                    that.modal = new ModalDialog({
                        ...
                        body: JST["views/templates/dedicated-notprocessedrows-dialog-bootstrap.ejs"]({__: i18n, data: notRows, file : data.source})
                    });
